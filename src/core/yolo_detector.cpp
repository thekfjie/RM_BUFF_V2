...
bool YoloDetector::loadModel() {
    ...
    std::cout << "[YoloDetector] Loading model: " << config_.modelPath
              << " (cwd=" << fs::current_path().string() << ")" << std::endl;
...
DetectionResult YoloDetector::detect(cv::Mat& frame) {
    ...
    // Tracker-only frames do not have current-frame pose keypoints. Keeping the
    // previous YOLO keypoints here would make downstream PnP use stale pixels.
...
bool YoloDetector::seedTracker(const cv::Mat& frame, const Parameter& param) {
    const std::optional<DetectionResult> detection = detectTarget(frame, config_.preferredClassId);
