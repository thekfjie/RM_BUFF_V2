...
BuffNode::BuffNode() : Node("buff_tracker_node") {
    ...
    targetPub_ = this->create_publisher<rm_buff_tracker::msg::BuffTarget>(
        "~/target", 10);
...
void BuffNode::loadRuntimeConfig() {
    ...
    yoloShowDebug_ = this->get_parameter("yolo_show_debug").as_bool();
    ...
    RCLCPP_INFO(this->get_logger(),
                "Runtime config: detector_type=%s, onnx_model_path=%s, yolo_show_debug=%s",
                parameter_.detectorType.c_str(),
                parameter_.onnxModelPath.c_str(),
                yoloShowDebug_ ? "true" : "false");
}
...
bool BuffNode::ensureYoloAssistLoaded() {
    ...
    RCLCPP_INFO(this->get_logger(), "Resolved ONNX model path: %s", yoloConfig.modelPath.c_str());

    yoloAssist_ = std::make_unique<YoloDetector>(yoloConfig, yoloShowDebug_);
...
void BuffNode::publishBuffTarget(const std_msgs::msg::Header& header,
                                 const PipelineOutput& output,
                                 bool found) const {
    auto targetMsg = rm_buff_tracker::msg::BuffTarget();
    targetMsg.header = header;
    targetMsg.tracking = found;
    targetMsg.prediction_ready = output.predictionReady;
    targetMsg.color = this->get_parameter("color").as_string();
    targetMsg.mode = this->get_parameter("mode").as_string();

    targetMsg.r_center_px.x = output.rBox.center2f().x;
    targetMsg.r_center_px.y = output.rBox.center2f().y;
    targetMsg.r_center_px.z = 0.0;
    targetMsg.fan_center_px.x = output.fanBladeBox.center2f().x;
    targetMsg.fan_center_px.y = output.fanBladeBox.center2f().y;
    targetMsg.fan_center_px.z = 0.0;
    targetMsg.aim_point_px.x = output.compensatedPoint.x;
    targetMsg.aim_point_px.y = output.compensatedPoint.y;
    targetMsg.aim_point_px.z = 0.0;

    targetMsg.radius_px = output.radius;
    targetMsg.observed_angle = output.observedAngle;
    targetMsg.raw_angle = output.rawAngle;
...
void BuffNode::imageCallback(const sensor_msgs::msg::Image::SharedPtr msg) {
    ...
    if (output.rBox.area() == 0.0) {
        ...
        publishBuffTarget(msg->header, output, false);
