...
std::optional<std::vector<FanBlade>> F_BuffTracker::getFanBlade(cv::Mat& mask) {
    ...
    for (std::size_t index = 0; index < realFanBladeList.size(); ++index) {
        if (realFanBladeList.size() == 1U) {
            ...
            const int id = realFanBladeList[index].bbox.id;
            if (states_[static_cast<std::size_t>(id)] == "target") {
                states_[static_cast<std::size_t>(id)] = "shot";
            } else if (states_[static_cast<std::size_t>(id)] == "unlighted") {
