...
上位机首次部署请看 [Ubuntu 22.04 / ROS 2 Humble 环境与部署步骤](docs/DEPLOYMENT_UBUNTU22_HUMBLE.md)，包含 C++ 推理 SDK、相机/TF、构建、启动和参数检查。

2026-09-18 源码更新：小符相位 KF、大符随机三点候选与内点重拟合、旋向/选靶修正、空间消息时域与坐标语义修正、PnP 深度有效期及断流失效。**本轮只做静态修改，未编译或运行验证。** 消息定义已变更，部署前必须重建节点与消费者。详见 [修改记录](docs/STATIC_CHANGES_2026-09-18.md) 和 [视觉接口约定](docs/VISUAL_INTERFACE.md)。
...
- 空间通道默认逐帧 YOLO 复核目标类别；兼容单节点仍保留 YOLO 初始化/重锁 + HSV 几何跟踪
...
注意：示例配置默认关闭 PnP。此时空间位置是把像素射线延伸到配置的 `target_distance`，属于固定距离近似，不是真实深度；实车使用前必须补齐真实相机标定、与当前 5 关键点模型严格对应的四个扇叶物理点和外参验证。启用后 PnP 深度只能在 TTL 内保持，过期即失效，不再回退固定距离。`position` 是当前观测，`predicted_position` 是未来点；两者时间差由 `prediction_horizon` 表示。不能直接套用其它 9 关键点模型的点序和尺寸，也不能把相机相对角当作绝对电机命令。
