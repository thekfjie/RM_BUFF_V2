...
更新：2026-09-18。队伍尚未决定最终由上位机还是下位机完成预测/弹道，本文件只确定视觉消息语义，不定义串口包或发射协议。当前修改尚未编译或运行验证。
...
已有 `rm_serial_driver` 订阅的是装甲板 `auto_aim_interfaces/Target`。不能把大小符消息 remap 过去，也不能把像素、扇叶相位或已经预测的点冒充装甲板旋转状态。
